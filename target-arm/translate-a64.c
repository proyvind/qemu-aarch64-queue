...
static void disas_fp_ccomp(DisasContext *s, uint32_t insn)
{
    ...
    if (cond < 0x0e) { /* not always */
        ...
        arm_gen_test_cc(cond, label_match);
        /* nomatch: */
        tcg_gen_movi_i64(tcg_flags, nzcv << 28);
        gen_set_nzcv(tcg_flags);
        tcg_gen_br(label_continue);
        gen_set_label(label_match);
    }
...
/* move to dst FP register a src FP register. Only single and double supp. */
static void gen_mov_fp2fp(int type, unsigned int dst, unsigned int src)
{
    TCGv_i64 tcg_src;
    tcg_target_long freg_offs_dst, freg_offs_src;
    freg_offs_dst = offsetof(CPUARMState, vfp.regs[dst * 2]);
    freg_offs_src = offsetof(CPUARMState, vfp.regs[src * 2]);

    tcg_src = tcg_temp_new_i64();
    switch (type) {
    case 0: /* single precision */
        tcg_gen_ld32u_i64(tcg_src, cpu_env, freg_offs_src);
        break;
    case 1: /* double precision */
        tcg_gen_ld_i64(tcg_src, cpu_env, freg_offs_src);
        break;
    default:
        assert(0); /* only single and double precision supported */
    }
    tcg_gen_st_i64(tcg_src, cpu_env, freg_offs_dst);
    tcg_gen_movi_i64(tcg_src, 0);
    tcg_gen_st_i64(tcg_src, cpu_env, freg_offs_dst + sizeof(float64));
...
static void disas_fp_csel(DisasContext *s, uint32_t insn)
{
    ...
    if (cond < 0x0e) { /* not always */
        ...
        arm_gen_test_cc(cond, label_match);
        /* nomatch: */
        gen_mov_fp2fp(type, rd, rm);
        tcg_gen_br(label_continue);
        gen_set_label(label_match);
    }
    /* match, or always */
    gen_mov_fp2fp(type, rd, rn);
...
static void disas_fp_3src_single(DisasContext *s, uint32_t insn)
{
    int rd = extract32(insn, 0, 5);
    int rn = extract32(insn, 5, 5);
    int ra = extract32(insn, 10, 5);
    ...
    if (o1 == true) {
        tcg_gen_mov_i32(tcg_res, tcg_op3);
        gen_helper_vfp_negs(tcg_op3, tcg_res);
    }

    if (o0 != o1) {
        tcg_gen_mov_i32(tcg_res, tcg_op1);
        gen_helper_vfp_negs(tcg_op1, tcg_res);
    }
...
static void disas_fp_3src_double(DisasContext *s, uint32_t insn)
{
    int rd = extract32(insn, 0, 5);
    int rn = extract32(insn, 5, 5);
    int ra = extract32(insn, 10, 5);
    ...
    if (o1 == true) {
        tcg_gen_mov_i64(tcg_res, tcg_op3);
        gen_helper_vfp_negd(tcg_op3, tcg_res);
    }

    if (o0 != o1) {
        tcg_gen_mov_i64(tcg_res, tcg_op1);
        gen_helper_vfp_negd(tcg_op1, tcg_res);
    }
...
static void disas_fp_3src(DisasContext *s, uint32_t insn)
{
    ...
    switch (type) {
    case 0:
        disas_fp_3src_single(s, insn);
        break;
    case 1:
        disas_fp_3src_double(s, insn);
        break;
    default:
        unallocated_encoding(s);
    }
...
static void disas_fp_int_conv(DisasContext *s, uint32_t insn)
{
    ...
    if (!sbit && (rmode < 2) && (opcode > 5)) {
        ...
        handle_fpfpcvt(s, insn, from_float, rmode);
    } else {
