...
/* dest = T0 - T1 - 1 + CF; do not compute flags. */
static void gen_sbc(int sf, TCGv_i64 dest, TCGv_i64 t0, TCGv_i64 t1)
{
    TCGv_i64 flag = tcg_temp_new_i64();
    tcg_gen_extu_i32_i64(flag, cpu_CF);
...
/* dest = T0 - T1 - 1 + CF; compute C, N, V and Z flags. */
static void gen_sbc_CC(int sf, TCGv_i64 dest, TCGv_i64 t0, TCGv_i64 t1)
{
    TCGv_i64 tmp = tcg_temp_new_i64();
    tcg_gen_not_i64(tmp, t1);
    gen_adc_CC(sf, dest, t0, tmp);
    tcg_temp_free_i64(tmp);
}
...
static void disas_adc_sbc(DisasContext *s, uint32_t insn)
{
    ...
    if (!setflags) {
        if (op) {
            gen_sbc(sf, tcg_rd, tcg_rn, tcg_rm);
        } else {
            gen_adc(sf, tcg_rd, tcg_rn, tcg_rm);
        }
    } else {
        if (op) {
            gen_sbc_CC(sf, tcg_rd, tcg_rn, tcg_rm);
        } else {
            gen_adc_CC(sf, tcg_rd, tcg_rn, tcg_rm);
        }
    }
}
...
static void disas_cc(DisasContext *s, uint32_t insn)
{
    ...
    int label_continue; TCGv_i64 tcg_tmp;
    ...
    {
        TCGv_i64 tcg_y, tcg_rn;
        if (is_imm) {
            tcg_y = new_tmp_a64(s);
            tcg_gen_movi_i64(tcg_y, y);
        } else {
            tcg_y = read_cpu_reg(s, y, sf);
        }
        tcg_rn = read_cpu_reg(s, rn, sf);
        if (op) {
